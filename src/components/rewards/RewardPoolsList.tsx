...
import {
    detectViewingKeyStatuses,
    batchQueryPoolBalances,
    hasValidBalance,
} from "../../utils/poolQueries";
import { setViewingKeys } from "../../utils/viewingKeys";
import {
    getRewardRedeemMessage,
    getRewardEmergencyRedeemMessage,
} from "../../utils/rewardExecutes";
...
export const RewardPoolsList: React.FC = () => {
    ...
    const loadViewingKeyStatuses = async () => {
        ...
        try {
            ...
            dispatch({
                type: "SET_ERROR",
                payload: "Failed to check viewing keys",
            });
        } finally {
    ...
    const handleSetViewingKeys = async () => {
        ...
        try {
            ...
            const poolsToSet = Array.from(state.selectedPoolAddresses).filter(
                (poolAddress) => {
                    const pool = state.rewardPools.find(
                        (p) => p.pool_address === poolAddress,
                    );
            ...
            if (poolsToSet.length === 0) {
                dispatch({
                    type: "SET_ERROR",
                    payload: "No pools selected that need viewing keys",
                });
                return;
            }
    ...
    const handleWithdraw = async () => {
        ...
        try {
            ...
            const poolsToWithdraw = Array.from(state.selectedPoolAddresses)
                ...
                .filter(
                    ({ pool, balance }) =>
                        pool &&
                        (pool.disabled ||
                            (balance && hasValidBalance(balance.balance))),
                );

            if (poolsToWithdraw.length === 0) {
                dispatch({
                    type: "SET_ERROR",
                    payload: "No pools selected with valid balances",
                });
                return;
            }
            ...
            const messages = poolsToWithdraw.map(({ pool, balance }) => {
                if (pool!.disabled) {
                    return getRewardEmergencyRedeemMessage(
                        state.walletAddress!,
                        pool!.pool_address,
                    );
                } else {
                    return getRewardRedeemMessage(
                        state.walletAddress!,
                        pool!.pool_address,
                        balance!.balance,
                    );
            ...
            dispatch({
                type: "SET_ERROR",
                payload: "Failed to process withdrawals",
            });
        } finally {
    ...
    const canWithdraw =
        poolsNeedingKeys.length === 0 &&
        (poolsWithValidKeys.length > 0 || disabledPools.length > 0);
        ...
        <div style={{ maxWidth: "1000px", margin: "0 auto", padding: "2rem" }}>
            <Card>
                <CardHeader>
                    ...
                    >
                        Select pools to withdraw from. Viewing keys will be set
                    ...
                    >
                        {state.rewardPools.map((pool) => (
                            <PoolRow
                                ...
                                viewingKeyStatus={
                                    state.viewingKeyStatuses[pool.pool_address]
                                }
                                isSelected={state.selectedPoolAddresses.has(
                                    pool.pool_address,
                                )}
                ...
                <CardFooter>
                    ...
                    >
                        ...
                        {canWithdraw && (
                            ...
                            >
                                Withdraw from Pools (
                                {poolsWithValidKeys.length +
                                    disabledPools.length}
                                )
                            </Button>
                        )}
                        ...
                        >
                            {state.selectedPoolAddresses.size} of{" "}
                            {state.rewardPools.length} pools selected
                        </div>
                    </div>
...
const WithdrawSummary: React.FC<{ results: WithdrawResult[] }> = ({
    results,
}) => {
